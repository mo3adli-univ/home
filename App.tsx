...
const MainPage: React.FC<MainPageProps> = ({ onNavigate, t, theme, adsAvailable }) => (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-8 bg-background dark:bg-background-dark">
        <div className="page-content w-full max-w-2xl text-center animation-running animate-fadeInUp">
            <div className="logo-container mb-8">
                {theme === ThemeEnum.LIGHT || (theme === ThemeEnum.AUTOMATIC && typeof window !== 'undefined' && !window.matchMedia('(prefers-color-scheme: dark)').matches) ? (
            ...
            <div className="choice-cards-container grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                <div 
                    ...
                    onClick={() => onNavigate('semester-calculator')}
                >
                ...
                <div 
                    className="choice-card bg-surface dark:bg-surface-dark p-8 rounded-xl shadow-md dark:shadow-dark border border-stroke dark:border-stroke-dark cursor-pointer transition-all duration-300 hover:transform hover:-translate-y-2 hover:shadow-strong dark:hover:shadow-dark_strong"
                    onClick={() => onNavigate('annual-calculator')}
                >
...
interface SemesterCalculatorPageProps {
    modules: StoredModuleData[];
    t: (key: string) => string;
    language: Language;
    onBack: () => void;
    onAddModule: () => void;
    onEditModule: (m: StoredModuleData) => void;
    onDeleteModule: (id: string) => void;
    onDeleteAll: () => void;
    onCalculate: () => void;
    isShakeDelete: boolean;
    adsAvailable: boolean;
}
const SemesterCalculatorPage: React.FC<SemesterCalculatorPageProps> = ({ 
    modules, t, language, onBack, onAddModule, onEditModule, onDeleteModule, onDeleteAll, onCalculate, isShakeDelete, adsAvailable 
}) => (
    <>
    <PageContainer titleKey="semester_gpa_title" onBack={onBack} t={t} language={language} hasFixedFooter>
        <div id="modules-list" className="mb-6">
            {modules.length === 0 ? (
                <p className="text-muted dark:text-muted-dark text-center py-8">{t('no_modules_yet')}</p>
            ) : (
                modules.map(module => (
                    <ModuleItem key={module.id} module={module} t={t} onEdit={() => onEditModule(module)} onDelete={() => onDeleteModule(module.id)} language={language}/>
                ))
            )}
        </div>
        {adsAvailable && <AdPlaceholder t={t} />}
    ...
    <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 dark:bg-background-dark/80 backdrop-blur-sm border-t border-stroke dark:border-stroke-dark z-10">
        <div className="max-w-2xl mx-auto space-y-2">
            <div className="flex items-center gap-2">
                <Button variant="add" onClick={onAddModule} className="flex-grow">
                    {t('add_new_module')}
                </Button>
                <Button
                    variant="danger"
                    onClick={onDeleteAll}
                    className={`flex-shrink-0 w-auto !p-3 ${isShakeDelete ? 'shake' : ''}`}
                    title={t('delete_all_modules')}
                    icon="fa-eraser"
                />
            </div>
            <Button variant="primary" onClick={onCalculate}>{t('show_result')}</Button>
        </div>
    </div>
    </>
);

interface AnnualCalculatorPageProps {
    s1Avg: string; s1Credits: string;
    s2Avg: string; s2Credits: string;
    t: (key: string) => string;
    language: Language;
    onBack: () => void;
    onInputChange: (field: 's1AvgText' | 's1CreditsText' | 's2AvgText' | 's2CreditsText', val: string) => void;
    onCalculate: () => void;
    onClear: () => void;
    onInfo: () => void;
    onNavigateSettings: () => void;
    isShakeClear: boolean;
    adsAvailable: boolean;
}
const AnnualCalculatorPage: React.FC<AnnualCalculatorPageProps> = ({ 
    s1Avg, s1Credits, s2Avg, s2Credits, t, language, onBack, onInputChange, onCalculate, onClear, onInfo, onNavigateSettings, isShakeClear, adsAvailable 
}) => {
        ...
        <div className="h-screen overflow-hidden flex flex-col relative">
                        ...
                        <div className="form-group mb-6">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                <Input 
                                    ...
                                    inputMode="decimal"
                                    value={s1Avg}
                                    onChange={(e) => onInputChange('s1AvgText', e.target.value)}
                                ...
                                <Input 
                                    ...
                                    inputMode="numeric"
                                    value={s1Credits}
                                    onChange={(e) => onInputChange('s1CreditsText', e.target.value)}
                                    placeholder={t('credits_placeholder')}
                                    disabled={parseFloat(s1Avg) >= 10}
                        ...
                        <div className="form-group mb-6">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                <Input 
                                    ...
                                    inputMode="decimal"
                                    value={s2Avg}
                                    onChange={(e) => onInputChange('s2AvgText', e.target.value)}
                                    placeholder={t('gpa_placeholder')}
                                    className="flex-grow" />
                                <Input 
                                    type="text" 
                                    inputMode="numeric"
                                    value={s2Credits}
                                    onChange={(e) => onInputChange('s2CreditsText', e.target.value)}
                                    placeholder={t('credits_placeholder')}
                                    disabled={parseFloat(s2Avg) >= 10}
                                    className="sm:flex-basis-[100px] sm:flex-grow-0" />
                            </div>
                        </div>
                         <p 
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 dark:bg-background-dark/80 backdrop-blur-sm border-t border-stroke dark:border-stroke-dark z-10">
                <div className="max-w-2xl mx-auto flex items-center gap-2">
                    <Button variant="primary" onClick={onCalculate} className="flex-grow">{t('show_result')}</Button>
                    <Button
                        variant="danger"
                        onClick={onClear}
                        className={`flex-shrink-0 w-auto !p-3 ${isShakeClear ? 'shake' : ''}`}
                        title={t('delete_all_modules')}
                        icon="fa-eraser"
                    />
                </div>
...
interface SettingsPageProps {
    appState: AppState;
    updateSettings: (s: Partial<AppState>) => void;
    onBack: () => void;
    t: (key: string) => string;
    onAddCustomMethod: () => void;
    onDeleteCustomMethod: (id: string) => void;
    onOpenModal: (type: ModalType) => void;
    initiallyOpen?: string | null;
    confirmModalCallback: (payload: ConfirmModalPayload) => void;
    feedbackRef: React.RefObject<HTMLTextAreaElement>;
    isSendingFeedback: boolean;
    onSendFeedback: () => void;
}
const SettingsPage: React.FC<SettingsPageProps> = ({ 
    appState, updateSettings, onBack, t, onAddCustomMethod, onDeleteCustomMethod, onOpenModal, initiallyOpen, confirmModalCallback, feedbackRef, isSendingFeedback, onSendFeedback 
}) => {
    ...
    return (
        <PageContainer titleKey="settings" onBack={onBack} t={t} language={appState.language}>
            <div>
                    ...
                    <div className="mt-2 space-y-2">
                        {allCalculationMethods.map(opt => (
                            <SettingOption 
                                ...
                                onDelete={('isCustom' in opt && opt.isCustom) ? () => onDeleteCustomMethod(opt.id) : undefined}
                                t={t}
                            >
                ...
                <SettingItem titleKey="set_credits" icon="fa-layer-group" t={t} language={appState.language} initiallyOpen={initiallyOpen === 'set_credits'}>
                ...
                <SettingItem titleKey="language" icon="fa-language" t={t} language={appState.language}>
                 ...
                 <div 
                    ...
                    onClick={() => onOpenModal('dataManagement')}
                >
                ...
                <div className="setting-item bg-surface dark:bg-surface-dark p-5 rounded-lg-custom mb-6 border border-stroke dark:border-stroke-dark cursor-pointer" onClick={() => onOpenModal('contact')}>
                     <div className="setting-item-header flex justify-between items-center">
                        <div className="flex items-center gap-3">
                ...
                <SettingItem titleKey="about_app" icon="fa-circle-info" t={t} language={appState.language}>
                    ...
                    <Button variant="primary" onClick={onSendFeedback} disabled={isSendingFeedback}>
                        {isSendingFeedback ? (
                ...
                <div className="setting-item bg-surface dark:bg-surface-dark p-5 rounded-lg-custom mb-6 border border-stroke dark:border-stroke-dark cursor-pointer" onClick={() => onOpenModal('privacy')}>
                     <div className="setting-item-header flex justify-between items-center">
                        <div className="flex items-center gap-3">
...
const App: React.FC = () => {
  ...
  const [navStack, setNavStack] = useState<PageId[]>(['main']);
  ...
  const navigateTo = (page: PageId, openSettingKey?: string) => {
    setNavStack(prev => [...prev, page]);
    if (openSettingKey) {
  ...
  const goBack = () => {
    setNavStack(prev => {
        ...
        setOpenSettingOnLoad(null); // Reset deep link setting on back
        return newStack;
    });
  };
    ...
    setModuleForm(prev => {
        ...
        if (field === 'tdGrade' || field === 'tpGrade' || field === 'examGrade') {
            ...
            if (!isNaN(numValue) && numValue > 20) {
                ...
                finalValue = strValue as ModuleFormData[K];
            }
        }
  ...
  const saveModule = () => {
    ...
    if (calculationMethod?.type === 'complex') {
        ...
        } else if (tdGradeProvided) {
            totalPoints += tdVal * continuousBaseWeight;
            totalWeight += continuousBaseWeight;
        } else if (tpGradeProvided) {
  ...
  const calculateSemesterResult = () => {
    ...
    lastSemesterResultRef.current = result;
    openResultModal({ 
    ...
    const handleAnnualInputChange = (field: 's1AvgText' | 's1CreditsText' | 's2AvgText' | 's2CreditsText', value: string) => {
        ...
        if (field.includes('AvgText')) {
            ...
            if (digits.length === 0) {
                ...
                if (num > 20) finalValue = '20.00';
                else {
                    if (relevantDigits.length > 2) finalValue = relevantDigits.slice(0, 2) + '.' + relevantDigits.slice(2);
                    else finalValue = relevantDigits;
                }
            }
        } else if (field.includes('CreditsText')) {
            const num = parseInt(value.replace(/\D/g, ''), 10);
            if (isNaN(num)) finalValue = '';
            else if (num > 30) finalValue = '30';
            else finalValue = String(num);
        }
  ...
  return (
    <>
      {currentPage === 'main' && <MainPage onNavigate={navigateTo} t={t} theme={appState.theme} adsAvailable={adsAvailable} />}
      
      {currentPage === 'semester-calculator' && (
        <SemesterCalculatorPage 
            modules={appState.modules} 
            t={t} 
            language={appState.language} 
            onBack={goBack} 
            ...
            onDeleteAll={deleteAllModules}
            onCalculate={calculateSemesterResult}
            isShakeDelete={isShakeDeleteAll}
            adsAvailable={adsAvailable}
        />
      )}
      
      {currentPage === 'annual-calculator' && (
        <AnnualCalculatorPage 
            s1Avg={appState.s1AvgText}
            s1Credits={appState.s1CreditsText}
            s2Avg={appState.s2AvgText}
            s2Credits={appState.s2CreditsText}
            t={t}
            language={appState.language}
            onBack={goBack}
            onInputChange={handleAnnualInputChange}
            onCalculate={calculateAnnualResult}
            onClear={clearAnnualInputs}
            onInfo={() => setActiveModal({ type: 'annualInfo' })}
            onNavigateSettings={() => navigateTo('settings', 'set_credits')}
            isShakeClear={isShakeClearAnnual}
            adsAvailable={adsAvailable}
        />
      )}
      ...
      {currentPage === 'settings' && (
        <SettingsPage 
            ...
            onBack={goBack}
            t={t}
            onAddCustomMethod={() => setActiveModal({type: 'addCustomMethod'})}
            onDeleteCustomMethod={handleDeleteCustomMethod}
            onOpenModal={(type) => setActiveModal({type})}
            initiallyOpen={openSettingOnLoad}
            confirmModalCallback={openConfirmModal}
            feedbackRef={feedbackTextareaRef}
